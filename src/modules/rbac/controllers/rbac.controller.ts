...
export class RbacController {
  ...
  listUsers = async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const actorIsSuperAdmin = request.user?.sub
        ? await this.permissionService.canAssignSuperAdminRole(request.user.sub)
        : false
      const users = await this.userRoleService.listUsersWithRoles(actorIsSuperAdmin)
  ...
  getUserRoles = async (
    ...
    try {
      const actorIsSuperAdmin = request.user?.sub
        ? await this.permissionService.canAssignSuperAdminRole(request.user.sub)
        : false
      const roles = await this.userRoleService.getUserRoles(request.params.id, actorIsSuperAdmin)
      return reply.send({
