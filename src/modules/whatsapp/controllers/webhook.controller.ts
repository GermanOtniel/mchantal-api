...
import type { InboundWebhookService } from '../services/inbound-webhook.service'
...
export class WebhookController {
  ...
  receive = async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      await this.inbound.handleWebhook(rawBody, request.headers, request.body)
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Webhook error'
      if (message.toLowerCase().includes('firma')) {
