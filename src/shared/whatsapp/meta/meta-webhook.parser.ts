...
function normalizeMessage(msg: MetaInboundMessage, contactName?: string): NormalizedMessage | null {
...
function normalizeStatus(st: MetaStatus): NormalizedMessageStatus | null {
  ...
  const map: Record<string, NormalizedMessageStatus['status'] | undefined> = {
  ...
  for (const entry of payload.entry ?? []) {
    for (const change of entry.changes ?? []) {
      ...
      for (const msg of value.messages ?? []) {
        ...
        if (normalized) events.push({ kind: 'message', message: normalized })
      }
      for (const st of value.statuses ?? []) {
        const normalized = normalizeStatus(st)
        if (normalized) events.push({ kind: 'status', status: normalized })
      }
    }
  }
  return events
